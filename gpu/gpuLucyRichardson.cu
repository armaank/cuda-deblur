...
void kernel_elementWiseDivision(const double *A, const double *B, double *C, const uint W, const uint H)
{
    ...
    for (int idx=start_idx; idx < max_val; idx += stride) 
        C[idx] = B[idx]==0 ? 99999999 : A[idx]/B[idx];
}
...
/**
  ...
  */
  void updateUnderlyingImg(const double *c, const double *g, const double *g_m, double *f, double *tmp1, double *tmp2,
     const uint W, const uint H, const uint filter_W, const uint filter_H)
  {
